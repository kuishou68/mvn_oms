...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 回显基础资料名称
            selectGna: function (datas, gco) {
                var actions = [];
                $.each(datas, function (index, base) {
                    if (base.gco == ('' + gco)) {
                        actions.push($.common.sprintf("<span class='%s'>%s</span>", "", base.gna));
                        return false;
                    }
                });
                return actions.join('');
        ...
        operate: {
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.JeeFast_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
